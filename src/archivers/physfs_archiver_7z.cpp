...
struct SZIPLookToRead {
   ...
   CLookToRead2 lookStream;  /* lzma sdk i/o interface (higher level). */
};
...
static void* SZIP_ISzAlloc_Alloc(ISzAllocPtr p, size_t size) {
   return allocator.Malloc(size ? size : 1);
} /* SZIP_ISzAlloc_Alloc */

static void SZIP_ISzAlloc_Free(ISzAllocPtr p, void* address) {
   if (address)
...
static SRes SZIP_ISeekInStream_Read(ISeekInStreamPtr p, void* buf, size_t* size) {
...
static SRes SZIP_ISeekInStream_Seek(ISeekInStreamPtr p, Int64* pos, ESzSeek origin) {
   SZIPLookToRead* stream = (SZIPLookToRead*) p;
   PHYSFS_Io* io = stream->io;
   PHYSFS_sint64 base;
...
static void szipInitStream(SZIPLookToRead* stream, PHYSFS_Io* io) {
   ...
   LookToRead2_INIT(&stream->lookStream);
   LookToRead2_CreateVTable(&stream->lookStream, False);
