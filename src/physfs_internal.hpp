...
#if defined(_MSC_VER) or defined(__MINGW32__) or defined(__WATCOMC__)
   #include <malloc.h>
#endif

#if defined(PHYSFS_PLATFORM_SOLARIS) or defined(PHYSFS_PLATFORM_LINUX) or defined(PHYSFS_PLATFORM_OGC)
   #include <alloca.h>
#endif

#if defined(PHYSFS_PLATFORM_LINUX) and not defined(_FILE_OFFSET_BITS)
...
/// Atomic operations. Increment/decrement operations return the final        
/// incremented/decremented value.                                            
#ifdef PHYSFS_PLATFORM_PLAYDATE
   #define PHYSFS_NEED_ATOMIC_OP_FALLBACK 1
#elif defined(_MSC_VER) and _MSC_VER >= 1500
   #include <intrin.h>
...
#define BAIL(e, r) return [&] { MetaPhysFS::Throw<e>(); return r; } ()
//#define BAIL_ERRPASS(r) return r
#define BAIL_IF(c, e, r) if (c) BAIL(e, r)
#define BAIL_IF_ERRPASS(c, r) if (c) return r
...
#if defined(PHYSFS_PLATFORM_WINDOWS) or defined(PHYSFS_PLATFORM_OS2)
   constexpr char __PHYSFS_platformDirSeparator = '\\';
#else
   constexpr char __PHYSFS_platformDirSeparator = '/';
#endif
 ...
 *
 * Call PHYSFS_setErrorCode() and return (nullptr) if the file can't be opened.
 */
 ...
 *  Just return nullptr if the standard routines will suffice. (see
 *  calculateBaseDir() in physfs.c ...)
 * Your string must end with a dir separator if you don't return nullptr.
 *  Caller will allocator.Free() the retval if it's not nullptr.
 */
char* __PHYSFS_platformCalcBaseDir(const char* argv0);

/*
 * Get the platform-specific user dir.
 * As of PhysicsFS 2.1, returning nullptr means fatal error.
 * Your string must end with a dir separator if you don't return nullptr.
 *  Caller will allocator.Free() the retval if it's not nullptr.
 */
 ...
 * Returning nullptr means fatal error.
 * Your string must end with a dir separator if you don't return nullptr.
 *  Caller will allocator.Free() the retval if it's not nullptr.
 ...
 *
 * Return (nullptr) if you couldn't create one. Systems without threads can
 *  return any arbitrary non-nullptr value.
 */
