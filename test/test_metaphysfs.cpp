...
static void cmd_tree_recursive(const char* prefix, const char* fullPath, const char* name, unsigned depth, PHYSFS_uint64* total_dir_count, PHYSFS_uint64* total_file_count) {
   char** rc;
   ...
   if (rc == NULL) {
      ...
      printf("\n");
      for (i = rc, file_count = 0; *i != NULL; i++, file_count++) {
         char* newFullPath = mpfsAloc<char>(strlen(fullPath) + strlen(*i) + 2);
...
static int cmd_tree(char* args) {
   PHYSFS_uint64 total_dir_count = 0, total_file_count = 0;
   if (*args == '\"') {
      args++;
      args[strlen(args) - 1] = '\0';
   }
...
static int cmd_getdirsep(char*) {
   printf("Directory separator is [%s].\n", PHYSFS_getDirSeparator());
   return 1;
}


static int cmd_getlasterror(char*) {
